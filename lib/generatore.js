 ...
 * @LastEditors: wujian
 * @LastEditTime: 2022-03-02 15:29:24
 */
...
class TemplateClass {
  ...
  // 下载模板
  downloadGitRepo(url, targetDir) {
    return new Promise((resolve, reject) => {
      download('a123648305/vue3', 'test', function (err) {
        if (err) {
          reject(err)
        } else {
          resolve('success')
  ...
  async create() {
    ...
    const message = chalk.green(
      `waiting download ${chalk.red(this.projectType)} template`
    )
